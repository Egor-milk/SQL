/* Имеется следующая функция, которую мы написали в разделе,
посвящённом, собственно, функциям: */

/*Задание:

Модифицировать функцию should_increase_salary 
разработанную в секции по функциям таким образом, 
...
Протестировать реализацию, передавая следующие значения 
аргументов (с - уровень "проверяемой" зарплаты, 
r - коэффициент повышения зарплаты):
...
begin
    IF min_salary > max_salary THEN
        RAISE EXCEPTION 'min_salary (%) is bigger than max_salary (%)', min_salary, max_salary
        USING ERRCODE = 'P0001';
    END IF;
    
    IF min_salary < 0 OR max_salary < 0 THEN
        RAISE EXCEPTION 'min_salary and max_salary must be at least 0'
        USING ERRCODE = 'P0002';
    END IF;
	
    IF increase_rate < 0.05 THEN 
        RAISE EXCEPTION 'increase_rate must be at least 0.05'
        USING ERRCODE = 'P0003';
    END IF;
